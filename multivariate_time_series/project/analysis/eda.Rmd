...
```{r irf-plot-daily}
png("analysis/assets/irf_no2_others.png", width = 800, height = 600)
# Response other pollutants to a shock in NO2
irf_no2_to_co <- vars::irf(
  var_model,
  impulse = "NO2", # The variable receiving the shock
  # response = "CO", # The variable whose response we observe
...
plot(
  irf_no2_to_co,
  main = "Impulse Response: NO2 shock to other pollutants",
  xlab = "Days Ahead",
  # ylab = "Response",
  # font size for half width plot
  cex.main = 2.5,
...
```{r fevd-daily}
png("analysis/assets/fevd_delhi.png", width = 800, height = 600)
...
p3 <- autoplot(fc_all_pollutants) +
  labs(
    title = paste(forecast_horizon, "-Day Ahead Forecasts for Delhi Pollutants (from VAR on full data)"),
    x = "Time", y = "Log1p(Concentration)"
  ) +
  theme_minimal()
