...
We will plot the average temperature curves for each city without smoothing to visualize the raw data.
...
The minimum lambda values were also the ones that minimized the GCV.
...
levels(smoothed_midday_df$City) <- city_names # ensure correct city names
...
The smoothing error is quite low at just above 0.5 C on average, indicating that the smoothing process has preserved the underlying temperature patterns well.

## Derivatives Heatmaps (City Averages)

We will compute the derivatives of the smoothed temperature surfaces with respect to day and hour, and visualize them as heatmaps.
...
Daily and hourly derivatives show a surprisingly stable pattern across the year, with some seasonal variations.

## Covariance Analysis and Heatmap

We'll analyze the covariance of hourly temperatures, treating days as replicates for a chosen city.
...
The first two principal components (PC1 and PC2) capture the main variability in the temperature surfaces across cities. PC1 captures the overall temperature trend, while PC2 seems to capture the seasonal variation in temperatures.

## Superpose FPCA Scores on Map of India

We will visualize the FPCA scores of the city-average temperature surfaces on a map of India, using the first two principal components.
...
Clearly, PC1 is related to the location of the city, with southern cities having lower PC1 scores (cooler temperatures) and northern cities having higher scores (warmer temperatures).
...
We will now perform FPCA on the yearly data using the previously optimized lambda values for the city-average data.
...
The cities are surprisingly well-separated in the PCA space, with most cities forming a distinct cluster of points across years.

## Clustering Analysis

We will perform clustering on the FPCA scores of the city-average temperature surfaces to identify groups of cities with similar temperature patterns.
...
Clustering analysis reveals two distinct groups of cities based on their temperature patterns. The first cluster includes cities with significantly less exposure to extreme temperatures (milder winters and summers), while the second cluster contains cities with more pronounced seasonal temperature variations.
...
We reject the null hypothesis of equal means at many points, most notably in the summer and winter months, especially around midday. This suggests that the temperature profiles of the two clusters are significantly different, particularly during extreme weather conditions.

## Functional Regression Modeling

In this section, we will fit a functional regression model to predict midday temperatures based on midnight temperatures. This is a function-on-function regression where both the response and predictor are functional data objects.
