...
# Example comparing raw and smoothed for a slice of days (days 150-170) at all hours
days_slice <- 150:170
...
mean(abs(Y_smoothed_city_avg_grid - Y))
```
...
# daily_mat_smoothed is 24 hours (rows) x 365 days (cols)
...
# FPCA on the bivariate functional data object for city averages (bifd_obj_city_avg)
# The "replicates" are the different cities.

# Extract coefficients: array [nbasis_day, nbasis_hour, n_cities]
...
# Flatten coefficients into a matrix [n_cities, nbasis_day * nbasis_hour]
...
# FPCA Loadings (eigenfunctions in coefficient form)
# pca_res_city_avg$rotation contains eigenvectors [nb_s*nb_t, n_components]
...
# Build the 3D array Y_yearly [day, hour, city_year_observation]
...
for (i in 1:nrow(city_year_combinations)) {
  ...
  # Pivot wider, ensuring all hour columns are present (h0 to h23)
  # Use values_fn to handle cases where a day-hour might be duplicated (shouldn't happen with distinct above)
  # Use values_fill to handle missing tempC for a given day/hour within a year (will be NA)
  current_matrix_wide <- matrix_data_subset %>%
    tidyr::pivot_wider(
      names_from = hour_of_day,
      values_from = tempC,
      names_prefix = "h",
      names_sort = TRUE, # Ensures h0, h1, ... h23 order
      values_fill = NA_real_ # Explicitly fill missing combinations with NA
    ) %>%
    dplyr::arrange(day_of_year) %>%
    dplyr::select(-City, -year, -day_of_year) # Keep only hour columns

  # Ensure the matrix has the correct dimensions (365x24) and column names
  # This is crucial if some hours are entirely missing for some days
...
fpca_scores_plot_yearly <- ggplot(scores_data_yearly, aes(x = PC1, y = PC2)) +
  ...
  geom_text_repel(
    aes(label = ifelse(Year == first(Year), Label, "")), # Show label only for the first year of each city
    size = 2.5, max.overlaps = Inf, box.padding = 0.5
  ) +
...
ggsave(file.path(params$output_dir, "figures/pca_yearly_scores.png"), fpca_scores_plot_yearly, width = 12, height = 8)
```
...
print(p_cluster_pca)
ggsave(file.path(params$output_dir, "figures/pca_clusters_city_avg.png"), p_cluster_pca, width = 10, height = 7)
```
...
print(p_cluster_map)
ggsave(file.path(params$output_dir, "figures/cluster_map_india.png"), p_cluster_map, width = 10, height = 9)
```
...
# Transform p-values: -log10(p-value)
# Add a small constant to p-values to avoid log10(0) if any p-value is exactly 0
neg_log10_p_value_matrix <- -log10(p_value_matrix + 1e-16)

# Determine a sensible range for the color scale, e.g., up to -log10(0.001) = 3
# or cap at a high percentile if there are extreme values.
...
p_pointwise_anova <- plot_heatmap_slope(
  ...
  days_vec = s_grid,
  hours_vec = t_grid,
  force_center = FALSE, # We want a sequential interpretation from 0 upwards
  value_range = c(0, max_val_plot) # Scale from 0 to max_val_plot
) + 
  scale_fill_gradientn(
    colors = c("lightgrey", "lightblue", "yellow", "orange", "red"), # More intuitive for significance
    limits = c(0, max_val_plot),
    name = "-log10(P-val)"
  ) +
  labs(subtitle = paste0("Comparison between Cluster 1 and Cluster 2. Higher values indicate greater significance.\nThreshold for p<0.05 is -log10(0.05) \u2248 1.3. Capped at ", round(max_val_plot,2), " (99th percentile)."))
...
We'll select one cluster and model how its daily mean temperature profile (averaged over hours) evolves across the years 2011-2018.

```{r functional_time_series_modeling}
# Data Preparation

# Add Cluster information to the yearly data scores
city_year_cluster_info <- scores_data_yearly %>%
  left_join(scores_df_city_avg_clustered %>% select(City, Cluster), by = "City")

# Model Cluster 1
target_cluster_id <- 1 
target_cluster_cities <- scores_df_city_avg_clustered %>% 
  filter(Cluster == target_cluster_id) %>% 
  pull(City)

cat("Selected Cluster for FTS:", target_cluster_id, "\n")
cat("Cities in Cluster", target_cluster_id, ":", paste(target_cluster_cities, collapse=", "), "\n")

years_for_fts <- 2011:2018
num_years_fts <- length(years_for_fts)

# Matrix to store the annual mean daily temperature profiles for the selected cluster
# Rows: days of the year (s_grid), Columns: years
cluster_annual_profiles_smoothed <- matrix(
  NA,
  nrow = length(s_grid),
  ncol = num_years_fts,
  dimnames = list(as.character(s_grid), as.character(years_for_fts))
)

cat("Preparing annual mean daily temperature profiles for Cluster", target_cluster_id, "...\n")
for (current_year in years_for_fts) {
  current_year_labels_in_cluster <- city_year_cluster_info %>%
    filter(Year == current_year, Cluster == target_cluster_id) %>%
    pull(Label)

  # Get indices of these city-year surfaces in the coefficient array of bifd_obj_yearly
  indices_in_coef_array <- which(city_year_labels %in% current_year_labels_in_cluster)
  
  # Calculate the mean coefficient matrix for this cluster and year
  mean_coefs_cluster_year <- apply(
    bifd_obj_yearly$coefs[, , indices_in_coef_array, drop = FALSE],
    c(1, 2), # Average over the 1st (day basis) and 2nd (hour basis) dimensions
    mean,
    na.rm = TRUE
  )
  
  # Create a bifd object for the mean surface of this cluster-year
  mean_surf_cluster_year_bifd <- bifd(
    coef = mean_coefs_cluster_year,
    sbasisobj = basis_day,
    tbasisobj = basis_hour
  )
  
  # Evaluate this mean surface on the s_grid (days) and t_grid (hours)
  evaluated_surf_cluster_year <- eval.bifd(s_grid, t_grid, mean_surf_cluster_year_bifd)
  
  # To get the mean daily temperature profile, average over the hours (t_grid, which is the 2nd dimension of evaluated_surf_cluster_year)
  daily_profile_cluster_year <- apply(evaluated_surf_cluster_year, 1, mean, na.rm = TRUE)
  
  cluster_annual_profiles_smoothed[, as.character(current_year)] <- daily_profile_cluster_year
}
cat("Finished preparing annual profiles.\n")

# Functional Time Series Modeling and Forecasting with ftsa

# Each row is a point on the functional curve (day of year).
fts_cluster_data <- fts(
  x = s_grid,
  y = cluster_annual_profiles_smoothed,
  start = min(years_for_fts),
  frequency = 1,
  xname = "Mean Daily Temperature Profile",
  yname = "Day of Year"
)

p_fts_data <- ggplot(data = fts_cluster_data, aes(x = x, y = y)) +
  # geom_line(aes(group = rownames(fts_cluster_data)), color = "grey", alpha = 0.5) +
  geom_line(stat = "summary", fun = "mean", color = "blue", size = 1.2) +
  labs(
    title = paste("Mean Daily Temperature Profiles - Cluster", target_cluster_id),
    x = "Day of Year",
...
# Plot the functional time series
plot_file_fts_data <- file.path(params$output_dir, paste0("figures/fts_cluster", target_cluster_id, "_profiles.png"))
png(plot_file_fts_data, width = 800, height = 600)
plot(fts_cluster_data, 
      main = paste("Annual Mean Daily Temperature Profiles - Cluster", target_cluster_id),
      xlab = "Day of Year", ylab = "Mean Temperature (°C)")
dev.off()
cat("Plotted FTS data to:", plot_file_fts_data, "\n")

num_pcs_fts <- 3
cat("Fitting FTS model with", num_pcs_fts, "principal components...\n")
model_fts_cluster <- ftsm(y = fts_cluster_data, order = num_pcs_fts, method = "classical")

print(model_fts_cluster)
cat("Variance explained by PCs:", model_fts_cluster$varprop, "\n")

# Plot model components (mean function and eigenfunctions)
plot_file_fts_model <- file.path(params$output_dir, paste0("figures/fts_cluster",target_cluster_id,"_model.png"))
png(plot_file_fts_model, width = 900, height = 700)
plot(model_fts_cluster, main_title = paste("FTS Model Components - Cluster", target_cluster_id))
dev.off()
cat("Plotted FTS model components to:", plot_file_fts_model, "\n")

# Forecast for the next year (2019)
cat("Forecasting 1 year ahead...\n")
forecast_fts_cluster <- forecast(model_fts_cluster, h = 1, level = 95)

# Plot the forecast
plot_file_fts_forecast <- file.path(params$output_dir, paste0("figures/fts_cluster",target_cluster_id,"_forecast.png"))
png(plot_file_fts_forecast, width = 800, height = 600)
plot(
  forecast_fts_cluster,
  main = paste("Forecast of Mean Daily Temperature Profile - Cluster", target_cluster_id, "for", max(years_for_fts)+1),
  xlab = "Day of Year",
  ylab = "Mean Temperature (°C)"
)
legend("topleft", legend=c("Historical Data", "Forecast Mean", "80% PI", "95% PI"), 
        col=c("grey", "blue", "lightblue", "lightcyan"), lty=c(1,1,2,2), lwd=c(1,2,1,1), cex=0.8)
dev.off()
cat("Plotted FTS forecast to:", plot_file_fts_forecast, "\n")
```
